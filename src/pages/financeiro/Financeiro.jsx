...
export default function Financeiro() {
  ...
  const transacoes = useMemo(() => listTransacoesStorage(), [])
  ...
  const handleVerComprovante = (doc) => window.alert(`Comprovante: ${doc.documento}\nTipo: ${doc.tipo}\nLançamento: ${doc.lancamento}\nProjeto: ${doc.projeto}\nValor: ${fmt(doc.valor)}\nStatus: ${doc.status}`)
...
function TransactionsTable({ transacoes, title }) { return <div className="card animate-fade-up delay-3"><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}><div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 16 }}>{title}</div><div style={{ display: 'flex', gap: 8 }}><button className="btn btn-sm btn-outline"><Download size={14} /> Exportar</button><button className="btn btn-sm btn-outline" onClick={() => window.print()}><Printer size={14} /> PDF</button></div></div><div className="table-wrap"><table><thead><tr><th>Descrição</th><th>Projeto</th><th>Categoria</th><th>Conta</th><th>Vencimento</th><th>Status</th><th>Comprovante</th><th style={{ textAlign: 'right' }}>Valor</th></tr></thead><tbody>{transacoes.map((t) => { const cfg = statusConfig[t.status] || statusConfig.PREVISTA; const Icon = cfg.icon; return <tr key={t.id}><td><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><div style={{ width: 32, height: 32, borderRadius: 8, background: t.tipo === 'RECEITA' ? 'var(--green-50)' : 'var(--red-50)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{t.tipo === 'RECEITA' ? <ArrowUpRight size={15} color="var(--green-600)" /> : <ArrowDownRight size={15} color="var(--red-600)" />}</div><div><span style={{ fontWeight: 600 }}>{t.descricao}</span><div style={{ fontSize: 12, color: 'var(--gray-400)' }}>{t.data ? new Date(`${t.data}T12:00:00`).toLocaleDateString('pt-BR') : '-'} • {t.forma}</div></div></div></td><td>{t.projeto}</td><td><span className="badge badge-gray">{t.categoria}</span></td><td>{t.conta}</td><td>{t.vencimento ? new Date(`${t.vencimento}T12:00:00`).toLocaleDateString('pt-BR') : '-'}</td><td><span className={`badge ${cfg.badge}`}><Icon size={11} /> {cfg.label}</span></td><td><span className={`badge ${t.comprovante === 'VALIDO' ? 'badge-green' : 'badge-yellow'}`}>{t.comprovante === 'VALIDO' ? 'Validado' : 'Pendente'}</span></td><td style={{ textAlign: 'right', fontWeight: 700, color: t.tipo === 'RECEITA' ? 'var(--green-600)' : 'var(--red-600)' }}>{t.tipo === 'RECEITA' ? '+' : '-'}{fmt(t.valor)}</td></tr> })}{transacoes.length === 0 && <tr><td colSpan="8" style={{ textAlign: 'center', color: 'var(--gray-400)' }}>Nenhuma transação cadastrada.</td></tr>}</tbody></table></div></div> }
function AccountsTable({ contas }) { return <div className="card"><CardTitle title="Contas financeiras" subtitle="Dados reais cadastrados" />{contas.length ? <div className="table-wrap"><table><thead><tr><th>Conta</th><th>Tipo</th><th>Banco</th><th>Saldo inicial</th><th>Saldo atual</th><th>Responsável</th><th>Status</th></tr></thead><tbody>{contas.map((conta) => <tr key={conta.id}><td><strong>{conta.nome}</strong></td><td><span className="badge badge-blue"><Landmark size={11} /> {conta.tipo}</span></td><td>{conta.banco}</td><td>{fmt(conta.saldoInicial)}</td><td><strong>{fmt(conta.saldoAtual)}</strong></td><td>{conta.responsavel}</td><td><span className="badge badge-green">Ativa</span></td></tr>)}</tbody></table></div> : <EmptyState texto="Nenhuma conta cadastrada." />}</div> }
function BudgetsTable({ orcamentos }) { return <div className="card"><CardTitle title="Orçamentos" subtitle="Dados reais cadastrados" />{orcamentos.length ? <div /> : <EmptyState texto="Nenhum orçamento cadastrado." />}</div> }
