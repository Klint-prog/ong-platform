...
export default function NovaTransacaoPage() {
  ...
  const salvar = (event) => {
    ...
    if (tipo === 'ORCAMENTO') {
      const novoOrcamento = addOrcamentoStorage({
        projeto,
        categoria,
        naturezaRecurso,
        descricao,
      ...
      // Cria comprovante vinculado ao orcamento para rastreabilidade
      addComprovanteStorage({
        documento: 'orcamento-' + String(descricao || 'orcamento').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') + '.txt',
        tipo: 'Orcamento financeiro',
        ...
        forma: conta,
        valor,
        status: status === 'APROVADA' ? 'VALIDO' : 'PENDENTE',
        validador: status === 'APROVADA' ? 'Admin' : '-',
